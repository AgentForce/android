...
public class PhonePresent extends BasePresenter<PhoneContract.View> implements PhoneContract.Action {
    ...
    public PhonePresent(PhoneContract.View presenterView, Context context) {
        super(presenterView);
    ...
    private void handleResponseCheckUser(CheckUser checkUser) {
        ...
        {
            SOPSharedPreferences.getInstance(mContext).saveAccessToken("Bearer " +checkUser.data.token);
