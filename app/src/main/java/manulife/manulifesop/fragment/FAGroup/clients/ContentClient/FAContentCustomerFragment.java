...
public class FAContentCustomerFragment extends BaseFragment<MainFAActivity, FAContentCustomerPresent> implements FAContentCustomerContract.View {
    ...
    private void calculatePerCentContractPerWeek(CampaignMonth data) {
        ...
        for (int i = 0; i < data.data.campaigns.size(); i++) {
            totalContract += data.data.campaigns.get(i).targetContractSale;
            mListContractWeek.add(data.data.campaigns.get(i).targetContractSale);
        }
    ...
    private void initAllViewsMonth(View view) {
        ...
        for (int i = 0; i < mData.data.campaigns.size(); i++) {
            targetstep1 += mData.data.campaigns.get(i).targetCallSale;
            targetstep2 += mData.data.campaigns.get(i).targetMetting;
            targetstep3 += mData.data.campaigns.get(i).targetPresentation;
            targetstep4 += mData.data.campaigns.get(i).targetContractSale;
            targetstep5 += mData.data.campaigns.get(i).targetReLead;
        }
