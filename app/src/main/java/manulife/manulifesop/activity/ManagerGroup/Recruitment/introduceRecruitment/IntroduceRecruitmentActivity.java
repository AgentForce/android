...
import manulife.manulifesop.fragment.ManagerGroup.recruiment.personalRecruitment.Introduce.IntroduceRecruitmentTabFragment;
import manulife.manulifesop.util.Contants;


public class IntroduceRecruitmentActivity extends BaseActivity<IntroduceRecruitmentPresenter> implements IntroduceRecruitmentContract.View {
    ...
    private boolean mIsFromRecruit;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_contact_person);
        mActionListener = new IntroduceRecruitmentPresenter(this, this);
        hideKeyboardOutside(layoutRoot);
        mTarget = getIntent().getIntExtra("target", 0);
        mMonth = getIntent().getIntExtra("month", 0);
        //variable is open from contact activity
        mIsFromRecruit = getIntent().getBooleanExtra("isFromRecruit",false);
        setupSupportForApp();
    ...
    public void initViewPager() {
        ...
        mListFragment.add(IntroduceRecruitmentTabFragment.newInstance(Contants.INTRODURE, mMonth));
